...
export default async function SearchPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <p className="text-[hsl(var(--muted-foreground))]">No results.</p>
      ) : (
        <ul className="divide-y divide-[hsl(var(--border))] rounded-lg border border-[hsl(var(--border))]">
          {items.map((v) => (
            <li key={v.cve_id} className="px-4 py-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
              ...
              <Link href={`/cve/${v.cve_id}`} className="font-mono font-medium no-underline">
                {v.cve_id}
              </Link>
              <KevBadge kev={v.kev} />
...
function Filters({ sp }: { sp: { q?: string; severity?: string; kev?: string; ecosystem?: string } }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs">
      ...
      <span className="text-[hsl(var(--muted-foreground))]">Ecosystem:</span>
      {["npm", "PyPI"].map((e) => {
